...
class Settings:
    ...
    fps = 60
    caption = "Susi Baka" # Wanted to do something with it but didn't get to do it
    max_nof_SuSs = 15 # The min. of obsticles on the screen
    timer_spawn = 0 # The timer for the spawn duration
    timer_resize = 0 # The timer for the resizeing
    game_points = 0 # The Point counter
    SuS_size_speed = 2 # Starting speed of the obsticles (Will get changed)
    normal_SuS_size_speed = 2 # Setback to the Starting speed (Will not get changed)
    player_speed = 5 # Player speed
    
    spawn_delay = 0.9 # The Start delay of the spawning obsticles
    max_size = 200
    delay_spawn = 1
    delay_resize = 2
    SuS_size = 5
...
class SuS(pygame.sprite.Sprite): # The obsticles
    def __init__(self):
        super().__init__()
        ...
        print(self.rect.x, self.rect.y, self.rect.size) # Testing spawning and resizeing
    ...
    def update(self):
        ...
        Settings.test1 +=0.15
        if Settings.test1 >= Settings.test2 and self.scale <= Settings.max_size : 
...
class Mouse(pygame.sprite.Sprite):
    def __init__(self, ):
        super().__init__()
        self.image_og = pygame.image.load(os.path.join(Settings.path_image, "hand.png")).convert_alpha()
...
class Game(object):
    ...
    def run(self):
        ...
        while self.running:
            ...
            self.draw()
            self.collison()
            #self.collide()  Is not needed in this little game (Will be handeld in on_click())
        # Quits the game
        pygame.quit()
    ...
    def update(self): # Everything that needs to be checked for each frame
        self.SuSs.update()
        self.mouse.update()
    ...
    def start(self): # Will be used at the start once
